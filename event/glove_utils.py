...
class Glove:
    ...
    def id(self, word):
        ...
        if word in self.vocab_id:
            ...
            return self.vocab_id[',']#如果不在vocab 返回，的embedding
    ...
    def transform(self, words, size=None):
        ...
        words = words.split(" ")
        if not size:
            return np.array([self.id(x) for x in words]) #删掉了if x in self.vocab_id
        else:
            if len(words) > size:
                words = words[:size]
            ...
            if words:
                ar = np.array(words)
                padded = np.pad(ar,pad_width=(0,size-len(words)), mode='constant', constant_values=-1)#以-1填充 [22091,-1,-1,-1,-1....]
                weights = (padded+1 != 0).astype(int) #[1,0,0,0,0....]
                weights = weights / len(words)
                padded = np.absolute(padded)#[22091,1,1,1,1....]
                return padded, weights#[22091,1,1,1,1....], [1,0,0,0,0....]
            else:
                return None, None
    ...
    def transform_eval(self, words, size=None):
        """Transform list of words to list of vocab ids
           Also return the weights for the embeddings
        """

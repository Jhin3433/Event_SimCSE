...
class Pooler(nn.Module):
    ...
    def forward(self, attention_mask, outputs , verb_location = None):  ##新加verb_location
        last_hidden = outputs.last_hidden_state#[flatten_sentence_num, sentence_length, emb_size]
...
def cl_forward(cls,
    ...
    pooler_output = pooler_output.view((batch_size, num_sent, pooler_output.size(-1))) # 返回一个句子pooler后的tensor，(bs, num_sent, hidden)

    # If using "cls", we add an extra MLP layer
    # (same as BERT's original implementation) over the representation.
    if cls.pooler_type == "cls" or "verv_emb": ##新加verb_location，train模式拿到verb处平均emb，eval模式取句子所有词的平均emb（详见Pooler）
        pooler_output = cls.mlp(pooler_output) #pooler_output.shape [batch_len, sent_len, emb_size]，一个句子通过pooler变成一个emb
    ...
    cos_sim = cls.sim(z1.unsqueeze(1), z2.unsqueeze(0)) #z1.shape [batch, emb] unsqueeze后 [batch, 1, emb] ;z2.unsqueeze(0)->[1,batch, emb]
    # Hard negative
    if num_sent >= 3:

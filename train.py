...
#----------------------------------------添加verb位置索引#----------------------------------------
        ...
        for index, sentence in enumerate(sentences):
            token = sentence.strip().split(' ')[-1].strip('.')  # 按照空格进行分词，最后一个词是verb词
            subword = tokenizer(token)['input_ids'][1:-1]#verb词对应的id值
            match_result = match(subword, sent_features['input_ids'][index])#verb词对应的id值 在sentence id中对应的位置。
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[ List[int], List[List[int]], torch.Tensor]] ]) -> Dict[str, torch.Tensor]:
            ...
            if "label_ids" in batch:
                ...
                del batch["label_ids"]
            #batch["input_ids"].shape - > [bathc,3,32] 一行三个句子，每个句子id数为32
            return batch
